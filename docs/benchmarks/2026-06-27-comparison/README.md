...
| --- | ---: | ---: | --- | --- | --- |
| [MeroZemory/realtime-voice-runtime](https://github.com/MeroZemory/realtime-voice-runtime) | 0 | 0 | 2026-06-27 | `9281f06` | - |
...
The realtime-voice-runtime 0.0s row is a warm-cache `uv sync --frozen` audit on this machine. It is kept for traceability, but it should not be used as a setup-time superiority claim.

| Repository | Final setup status | Duration | Environment / command type | Evidence tail |
| --- | --- | ---: | --- | --- |
| MeroZemory/realtime-voice-runtime | pass | 0.0s | warm-cache `uv sync --frozen` audit | Audited 51 packages in 0.04ms |
...
Only realtime-voice-runtime produced numeric latency artifacts under a repeatable local harness in this run:
...
A direct latency bake-off against the reference repos would be misleading here: the updated reference repos do not expose a common non-interactive fixture equivalent to `speech_end/input commit -> first audio`, and their runtime defaults differ materially (full VTuber app, browser app, local cascade, Twitch/OBS integration, or GPU/local model stack). The fair result is therefore: realtime-voice-runtime has measured latency artifacts for its own ablation decisions; the others were setup-tested and compared by architecture/readiness, but not assigned invented latency numbers.

## Architectural Comparison

| Project | Default emphasis | Voice path | Barge-in / interrupt | Benchmark readiness in this run |
| --- | --- | --- | --- | --- |
| realtime-voice-runtime | CLI realtime voice core | OpenAI Realtime GA audio-native default | Implemented and measured | Numeric latency artifacts generated |
...
realtime-voice-runtime is new and has no meaningful public star history yet. This chart is for context, not runtime quality.
